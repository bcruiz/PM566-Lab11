...
The range of dates starts as early as January 1st until October 27th. The range of cases is very vast with the number of cases being the running grand total from every day. The same goes for the number of deaths as the recorded number of deaths is a running total ranging from 0 to 33,092 deaths.
...
### FINISH THE CODE HERE ###
p1<-ggplot(cv_states, aes(date, new_cases, color = state))+
  geom_line()+
...
### FINISH THE CODE HERE ###
p2<-ggplot(cv_states, aes(date, new_deaths, color = state))+
  geom_line()+
  geom_point(size = .5, alpha = 0.5)
ggplotly(p2)
...
The new_cases line plot visual has recorded cases that are in the negatives, such as the state of Massuchusetts in September 2nd with -7757 cases. For our new_cases variable we do have some extraneous outliers with the state of Texas on September 1st with 22,276 new cases and Georgia on October 5th with 21,349 new cases. For our variabl new_deaths we do have some negative values, which is not obtainable. There are some outliers with the state of New Jersey with 1,877 new deaths on June 25th.
...
cv_states$per100k = as.numeric(format(round(cv_states$cases/(cv_states$population/100000),1),
                                      nsmall=1))
cv_states$newper100k = as.numeric(format(round(cv_states$new_cases/(cv_states$population/100000),1),
                                         nsmall=1))
cv_states$deathsper100k = as.numeric(format(round(cv_states$deaths/(cv_states$population/100000),1),
                                            nsmall=1))
cv_states$newdeathsper100k = as.numeric(format(round(cv_states$new_deaths/(cv_states$population/100000),1),
                                               nsmall=1))
...
### FINISH THE CODE HERE ###
p <- ggplot(cv_states_today_scatter, aes(x=pop_density, y=newdeathsper100k, size=population))+
  geom_point()+
  geom_smooth(method = 'lm') 
ggplotly(p)
```

For our variables of pop_density on newdeathsper100k we see that there is a slight linear correltation. With the clustering of points towards the lower end of pop_density our linear correlation with the new deaths per 100k may be weak.
...
The peak of new cases is on September 21st with 22,276 recorded within the United States. The peak of deaths was recorded on July 27th with a record of 1,202. There was about a 2 months time delay difference between the two different peaks. 
...
fig <- fig%>%
  layout(title = paste('CFR by State as of', Sys.Date(), '<br>(Hover for value)'),
    geo = set_map_details)
fig_May1 <- fig
...
# Create the map
fig <- plot_geo(cv_CFR, locationmode = 'USA-states')%>% 
  add_trace(z = ~naive_CFR, text = ~hover, locations = ~state,
    color = ~naive_CFR, colors = 'Purples')
